...
function valueOf(row: HTMLTableRowElement, index: number): number {
...
function enhance(table: HTMLTableElement): void {
	...
	function apply(column: Column, dir: Direction): void {
	...
	function toggle(column: Column): void {
		...
		const next: Direction =
			activeKey === column.key && direction === "descending" ? "ascending" : "descending";
		apply(column, next);
	}

	function register(key: string, button: HTMLButtonElement): void {
		const list = buttons.get(key);
		if (list) list.push(button);
		else buttons.set(key, [button]);
	}
...
export function initTableSort(): void {
	for (const table of Array.from(document.querySelectorAll("table[data-sortable]"))) {
		enhance(table as HTMLTableElement);
	}
}
