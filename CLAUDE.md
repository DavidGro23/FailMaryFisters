...
2. **`matchup-history.json` contains playoff and consolation games too.** Find them by whether the `matchupId` appears in `playoff-history.json` — but **membership only tells you the game is post-regular-season, not that it is a playoff game.** That file holds two brackets; split them with rule 20. Summing both files double-counts the playoff weeks — which are 16–17 in most seasons but **15–16 in 2018–2020**, so never hardcode them either.
...
18. **Never derive *which* players were kept. Keeper *values* are a different thing and are computed.** Past keepers are indistinguishable from ordinary draft picks — the obvious heuristic was tested on 2024 → 2025 and produced 40 candidates with 8 of 10 teams over the limit — so that derivation stays forbidden. What the `/keepers/` page computes is the **price** of keeping a currently-rostered player, which is a stated rule rather than an inference: drafted → `round − 1` (first-rounders cannot be kept), undrafted → round 10 off waivers or round 12 as a free agent. **Trades and drops never change it**: a drafted player keeps his draft-based value however many teams he passed through and however often he was cut, so the draft is searched by `playerId` alone. The waiver/free-agent half is not in the export — see rule 21.
...
20. **"Playoff game" means the `Championship` bracket only — consolation games are not playoff games.** `playoff-history.json` holds exactly 8 games in every one of the nine seasons: 4 `bracketType: "Championship"` (two semifinals, the final, the third-place game) and 4 `bracketType: "Consolation"` (an unlabelled first round, plus the 5th- and 7th-place games). **The league does not count the consolation bracket as playoffs**, so `GameType` has three values — `regular`, `playoff`, `consolation` — and consolation games belong to none of the displayed scopes. Never make them `regular` either: that would corrupt every regular-season record and W-L. Discriminate on `bracketType`, never `roundLabel` (rule 13). This was a real defect: classifying by mere membership in the file gave the lowest-playoff-score record to a 2017 consolation game (54.04) instead of the true 66.40, and gave a manager who has never reached the playoffs a full set of "postseason" records. Prefer the word *playoff* over *postseason* in code and copy — "postseason" reads as "everything after week 15" and is exactly the ambiguity that caused the bug.

21. **The export has no waiver, free-agent, add or drop records — only trades.** Every field at every nesting depth across all ten files × nine seasons was searched: `trade-history.json` is the only transaction data in it. `go-nfl-fantasy`, which produced this export, has scrapers for eleven data types and none for transactions. This matters because rulebook §5.3 prices an undrafted keeper on his last waiver/free-agent claim, and **44 of the 150 players on the 2025 rosters (29%) are undrafted**. That data is captured separately from the league's own transaction log and committed as `raw-data/<leagueFolder>/keeper-acquisitions.json` (`{ "<year>": { "<playerId>": "waiver" | "freeAgent" } }`). Its absence is expected and never an error — unpriced players render "not recorded". **Never default an undrafted player to 10 or 12**; a plausible wrong value is worse than a visible gap.

21a. **How that capture works, since it is not reproducible from anything in this repo.** `https://fantasy.nfl.com/league/<id>/history/<year>/transactions` accepts **only** `transactionType` values `add`, `drop`, `trade`, `roster`, `commish` — the page's own links enumerate them. **There is no `waiver` type**, and an unrecognised value silently falls back to a default view rather than erroring, so probing for one returns a plausible page of the wrong data. Waiver claims live inside `add`, distinguished by the **`transactionFrom` column, which is exactly `"Waivers"` or `"Free Agents"`**. Rows carry the player id in a `playerNameId-<id>` class, paginate via `offset=` in steps of 20, and arrive **newest-first** — so the first `add` seen for a player is his last acquisition, which is the one that prices him. Verify that ordering rather than trusting it. The tool that does this is **deliberately not in this repository**: the site needs no network code to build (rule 19, NFR-9), so it lives in the gitignored `local/`, reads the cookie from a `.env` outside the repo, and only its output is committed.
