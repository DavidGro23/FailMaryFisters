...
14a. **Negative DEF scores are stored as their absolute value.** 56 records across 2017–2024. Stage 2 recomputes the DEF score from that season's `dstSettings` and negates the exported value only when the recomputation is negative *and* the export equals its magnitude — never in any other case, never for any other position, and never touching the team total. **Guard first: an all-zero stats row means "did not play", not a shutout** — `2022-17-5-6` (the cancelled Bills–Bengals game) would otherwise score +10 off the `pts_allow_0` tier. See D17 and D18.
