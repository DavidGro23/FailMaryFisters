...
| D6 | **`matchup-history` points are authoritative.** Never recompute a team score from player stats. Measured across all nine seasons: **72 team-games** do not reconcile against the sum of their starters — 2017:6, 2018:9, 2019:7, 2020:8, 2021:5, 2022:9, 2023:11, 2024:15, 2025:2. **Every delta has the same sign: `matchupPoints − starterSum` is always negative**, i.e. the starters sum to *more* than the official team total, never less. These are **two distinct populations**: **56 of the 72 are the DEF sign-loss bug (D17)** — a deterministic, provable defect with the closed form `delta = −2 × stated DEF points` — and stage 2 corrects those. The remaining **16** have no structural explanation and are consistent with post-hoc NFL stat corrections; they are mostly fractional (0.20, 0.36, 0.40, 0.60, 0.80, 1.20, 1.40, 1.80, 2.40), plus two at −2.00 and the two 2025 cases (2025 has no DEF slot, so D17 cannot apply there). Separately, two lineups carry one starter fewer than that season's slot count: `2017-15-2-3` (team 3) and `2024-14-3-5` (team 3). Unreconciled games go to the validation report; **the team total in `matchup-history.json` is never altered, under any circumstance.** |
...
| D17 | **Negative DEF scores lose their sign in the export.** When a defense's true score is negative, `player-matchup-statistics-history.json` stores its **absolute value**. Measured over all 1298 DEF records (2017–2024): 1240 match a recomputation from that season's `dstSettings` with the sign intact, **56 match only in absolute value**, and 2 are unexplained. Because the team total in `matchup-history.json` is correct, a sign-flipped DEF makes the starter sum overshoot by exactly `2 × the stated DEF points` — which is the entire mechanism behind 56 of the 72 D6 mismatches. Example: `2017-10-4-9` team 4, Denver allowed 41 (`pts_allow_35p: -4`) with 1 sack (`+1`), true score **−3.00**, exported as **3.00**; with the DEF at −3.00 the nine starters sum to 46.94, exactly the official total. **Correction rule for stage 2:** recompute the DEF score from that season's `dstSettings` (with `def_st_td` at 6, per D12); if the result is negative *and* the exported value equals its absolute value, negate the exported value. Apply it in no other case, never recompute any other position, and **never alter the team total**. Log every correction to the validation report. Applies to 2017–2024 only; 2025 has no DEF slot. |
...
| 15 | Decide the display treatment on `/about/` for the **16** genuinely unreconciled team-games that remain after the D17 sign correction — one summary line, or a full list | Stage 4 |
